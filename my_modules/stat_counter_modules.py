...
class StatCalculatorModule :
    def __init__ (self, set_of_ues, duration, lambda_threshold) :
    ...
    def display (self) :
        ...
        #LSRL values
        for ue in self .set_of_ues :
            print ("UE :", ue, "LSRL_count :", self .LSRL_counter (ue), "Handover count :", self .handover_counter (ue))
    ...
    def save (self, outdir, prefix):
        ...
        for suffix, obj in zip (["connection", "lba_connection", "lba_before", "lba_after"], 
            ...
            debugging .log ("Writing to file", outfile)
            io .pickle_dump (obj, outfile)
            debugging .log ("Write complete")
            outfilenames .append (outfile)

        return outfilenames

    @debugging .trace (level = 1)
    def update_stat_lba_before (self ,t, val):
    ...
    @debugging .trace (level = 2)
    def load_handover_counter (self, time) :
        return len (self .stat_lba_connection [time])
