...
class UEPositionProviderModule :
    def __init__ (self, journey_length, ue_start_times, ue_pos) :
    ...
    def ue_still_in_system (self, ue, time) :
        ...
        debugging .log ("UE", ue, "Start time", start_time, "Current time", time)
    ...
    def get_ue_position (self, ue, time) :
        if self .ue_still_in_system (ue, time) :
            debugging .log ("UE is in system")
            return time - self .get_ue_start_time (ue) + 1
        else :
            debugging .log ("UE not in system")
            return None
