...
class SimulatorModule :
    def __init__ (  self, all_conf_file = ""):
        ...
        self .ue_pos_6kmph          = io .read_from_file_basic (self .source_dir, self .data_file_names ['ue_pos_6kmph'])
        ...
        self .rsrp_meas_6kmph       = io .read_from_file_basic (self .source_dir, self .data_file_names ['rsrp_meas_6kmph'])
        self .rsrq_meas             = io .read_from_file_basic (self .source_dir, self .data_file_names ['rsrq_meas'])
        self .rsrq_meas_6kmph       = io .read_from_file_basic (self .source_dir, self .data_file_names ['rsrq_meas_6kmph'])
        ...
        self .ue_start_times_6kmph  = io .read_from_file_basic (self .source_dir, self .all_config ['ue_start_times_6kmph_prefix'] +
                                                                                    str (self .common_config ['number_ue_6kmph']) + '_' +
                                                                                    str (self .all_config ['ue_start_times_set_no']) +
                                                                                    self .all_config ['ue_start_times_suffix'])

        self .preprocessed_data             = io .read_from_file_basic (self .source_dir, self .data_file_names ["preprocess_data"])
        self .preprocessed_data_6kmph       = io .read_from_file_basic (self .source_dir, self .data_file_names ["preprocess_data_6kmph"])
        
        #Adjustment of indexes of the UEs
        #for k in sorted (list (self .ue_start_times_6kmph .keys ()), reverse = True):
        #    self .ue_start_times_6kmph [k + len (self .ue_start_times)] = self .ue_start_times_6kmph [k]
        #    del (self .ue_start_times_6kmph [k])
        ...
        self .set_of_ues_6kmph      = None

        self .handover_module       = None
        self .handover_6kmph_module = None
        ...
        self .rsrpmp_6kmph_module   = None
        self .rsrqmp_module         = None
        self .rsrqmp_6kmph_module   = None
        self .uepp_module           = None
        self .uepp_6kmph_module     = None
        self .enbpp_module          = None

    def __enter__ (self):
        debugging .log ("__enter__ Entering Simulator Module")
        return self

    def __exit__ (self, exception_type, exception_value, traceback):
        debugging .log ("__exit__ Exiting Simulator Module")
    ...
    def simulate_at_time (self, t_now):
        ...
        for ue in self .set_of_ues :
            ...
            print ("Time", t_now, "UE", ue)
            if ue in self .uepp_module .get_set_of_ues () and not self .uepp_module .ue_still_in_system (ue, t_now):
                #skip as journey is not yet started or journey has finished
                debugging .log("Ignoring fast UE", ue, "at time", t_now, "as journey start time is", self .ue_start_times [ue])
                continue
            elif ue in self .uepp_6kmph_module .get_set_of_ues () and not self .uepp_6kmph_module .ue_still_in_system (ue, t_now):
                #skip as journey is not yet started or journey has finished
                debugging .log("Ignoring slow UE", ue, "at time", t_now, "as journey start time is", self .ue_start_times_6kmph [ue])
                continue

            elif ue in self .ue_start_times_6kmph and t_now == (self .ue_start_times_6kmph [ue] + self .common_config ['journey_length_6kmph']) :
                ...
                continue
            elif ue in self .ue_start_times and t_now == (self .ue_start_times [ue] + self .common_config ['journey_length']) :
                #clean up UE from the system as the journey has ended
                self .he_module .execute_handover (t_now, ue, self .uecs_module .get_serving_enb_of (ue, t_now), -1)
                continue
            
            serving = self .uecs_module .get_serving_enb_of (ue, t_now)
            if serving == -1 and self .lb_config ['name'] in ['jula', 'jula-dual']:
                debugging .log ("Time", t_now, "New Join UE", ue, "60kmph")
            ...
            if self .ho_config ['name'] != 'none':
                ...
                if (src != -1) and (dst != -1):
                    #update stats
                    rsrp, dbm = self .rsrpmp_module .get_reading_for (ue, t_now, dst) if ue in self .uepp_module .get_set_of_ues () else self .rsrpmp_6kmph_module .get_reading_for (ue, t_now, dst)
                    self .sc_module .update_stat_connection (t_now, ue, (src, dst, rsrp, dbm))

                self .he_module .execute_handover (t_now, ue, src, dst)

        '''
        for ue in self .set_of_ues_6kmph :
            debugging .log ("Time", t_now, "UE", ue)
            if t_now < self .ue_start_times_6kmph [ue] or t_now > (self .ue_start_times_6kmph [ue] + self .common_config ['journey_length_6kmph']):
                #skip as journey is not yet started or journey has finished
                debugging .log("Ignoring UE", ue, "at time", t_now, "as journey start time is", self .ue_start_times_6kmph [ue])
                continue

            elif t_now == (self .ue_start_times_6kmph [ue] + self .common_config ['journey_length_6kmph']) :
                #clean up UE from the system as the journey has ended
                self .he_module .execute_handover (t_now, ue, self .uecs_module .get_serving_enb_of (ue, t_now), -1)
                continue
            ...
            serving = self .uecs_module .get_serving_enb_of (ue, t_now)
            if serving == -1 and self .lb_config ['name'] in ['jula', 'jula-dual']:
                debugging .log ("Time", t_now, "New Join UE", ue, "6kmph")
                self .load_module .new_join (ue)

            if self .ho_config ['name'] != 'none':
                #engage handover module
                (time, ue, src, dst) = self .handover_6kmph_module .run (t_now, ue, serving)

                #Process handover module output
                if (src != -1) and (dst != -1):
                    #update stats
                    rsrp, dbm = self .rsrpmp_6kmph_module .get_reading_for (ue, t_now, dst)
                    self .sc_module .update_stat_connection (t_now, ue, (src, dst, rsrp, dbm))
        ...
        if t_now != 0 and t_now % self .common_config ['load_check_interval'] == 0:
            ...
            elif self .lb_config ['name'] in ['jula-dual', 'maxflow', 'jula', 'uara', 'dlba', 'none'] :
            ...
            for t, u, s, d in lba_transfers :
                ...
                #update stats
                rsrp, dbm = self .rsrpmp_module .get_reading_for (u, t, d) if u in self .uepp_module .get_set_of_ues () else self .rsrpmp_6kmph_module .get_reading_for (u, t, d)
    ...
    def configure (self):
        ...
        self .uepp_module       = pp_mod .UEPositionProviderModule (self .common_config ['journey_length'], 
                                                                    self .ue_start_times, 
                                                                    self .ue_pos)
        self .uepp_6kmph_module = pp_mod .UEPositionProviderModule (self .common_config ['journey_length_6kmph'], 
                                                                    self .ue_start_times_6kmph, 
                                                                    self .ue_pos_6kmph)
        ...
        self .rsrpmp_6kmph_module   = mp_mod .RSRPMeasurementProviderModule (     self .rsrp_meas_6kmph,
                                                                                self .enbpp_module, 
                                                                                self .uepp_6kmph_module,
                                                                                self .common_config ['lambda'])
        self .rsrqmp_module         = mp_mod .RSRQMeasurementProviderModule (   self .rsrq_meas,
                                                                        self .enbpp_module,
                                                                        self .uepp_module,
                                                                        self .common_config ['lambda'])
        self .rsrqmp_6kmph_module   = mp_mod .RSRQMeasurementProviderModule (     self .rsrq_meas_6kmph,
                                                                                self .enbpp_module,
                                                                                self .uepp_6kmph_module,
                                                                                self .common_config ['lambda'])

        #inititalize set of ues and set of enbs
        self .set_of_ues        = self .uepp_module .get_set_of_ues () .union (self .uepp_6kmph_module .get_set_of_ues ())
        debugging .log ("Set of UEs", self .set_of_ues, "\n6kmph", self .uepp_6kmph_module .get_set_of_ues (), "\n60kmph", self .uepp_module .get_set_of_ues ())
        #self .set_of_ues_6kmph  = self .uepp_6kmph_module .get_set_of_ues ()
        self .set_of_enbs       = self .enbpp_module .get_set_of_enbs ()
        debugging .log ("Set of eNBs", self .set_of_enbs)
    ...
    def choose_handover_module (self):
        if self .ho_config ['name'] == 'ussl-a' or self .ho_config ['name'] == 'ussl-c':
            ...
            self .handover_6kmph_module = hoa_mod .UEASHandoverModule (   self .ho_config, 
                                                                    self .common_config,
                                                                    uepp_obj = self .uepp_6kmph_module,
                                                                    rsrpmp_obj = self .rsrpmp_6kmph_module)
            ...
            self .handover_module = hoa_mod .A3HandoverModule (     self .ho_config, self .common_config, 
                                                                    uepp_obj = self .uepp_module,
                                                                    rsrpmp_obj = self .rsrpmp_module)
            self .handover_6kmph_module = hoa_mod .A3HandoverModule (     self .ho_config, self .common_config, 
                                                                    uepp_obj = self .uepp_6kmph_module,
                                                                    rsrpmp_obj = self .rsrpmp_6kmph_module)
            ...
            self .handover_module = hoa_mod .OMBFRAHandoverModule ( self .ho_config, self .common_config,
                                                                    ...
                                                                    rsrpmp_obj = self .rsrpmp_module)
            self .handover_6kmph_module = hoa_mod .OMBFRAHandoverModule ( self .ho_config, self .common_config,
                                                                    uepp_obj = self .uepp_6kmph_module,
                                                                    enbpp_obj = self .enbpp_module,
                                                                    enbcs_obj = self .enbcs_module,
                                                                    rsrpmp_obj = self .rsrpmp_6kmph_module)
            ...
            self .handover_module = hoa_mod .UBBHOHandoverModule (  self .ho_config, self .common_config,
                                                                    ...
                                                                    enbpp_obj = self .enbpp_module,
                                                                    rsrpmp_obj = self .rsrpmp_module)
            self .handover_6kmph_module = hoa_mod .UBBHOHandoverModule (  self .ho_config, self .common_config,
                                                                    uepp_obj = self .uepp_6kmph_module,
                                                                    enbpp_obj = self .enbpp_module,
                                                                    rsrpmp_obj = self .rsrpmp_6kmph_module)
            ...
            self .handover_6kmph_module = hoa_mod .A2A4HandoverModule (   self .ho_config, self .common_config,
                                                                    uepp_obj = self .uepp_6kmph_module,
                                                                    rsrqmp_obj = self .rsrqmp_6kmph_module)
            ...
            self .handover_module = hoa_mod .AttachUEModule (       self .ho_config, self .common_config,
                                                                    uepp_obj = self .uepp_module,
                                                                    rsrpmp_obj = self .rsrpmp_module)
            self .handover_6kmph_module = hoa_mod .AttachUEModule (       self .ho_config, self .common_config,
                                                                    uepp_obj = self .uepp_6kmph_module,
                                                                    rsrpmp_obj = self .rsrpmp_6kmph_module)
            ...
            self .handover_6kmph_module = hoa_mod .NoHandoverModule (  self .ho_config, self .common_config)
        else :
    ...
    def choose_load_module (self) :
        if self .lb_config ['name'] == 'none' :
            ...
            self .load_module = lba_mod .JointUEASLoadModule (   self .lb_config, self .common_config, 
                                                                        ...
                                                                        uecs_obj = self .uecs_module, 
                                                                        enbcs_obj = self .enbcs_module)
        elif self .lb_config ['name'] == 'jula-dual' :
            debugging .log ("LB_USSL_DUAL_Module is chosen for the codename --", self .lb_config ['name'])
            self .load_module = lba_mod .LB_USSL_DUAL_Module (   self .lb_config, self .common_config, 
                                                                        uepp_obj = self .uepp_module,
                                                                        uepp_6kmph_obj = self .uepp_6kmph_module,
                                                                        rsrpmp_obj = self .rsrpmp_module, 
                                                                        rsrpmp_6kmph_obj = self .rsrpmp_6kmph_module, 
                                                                        uecs_obj = self .uecs_module, 
                                                                        enbcs_obj = self .enbcs_module,
                                                                        preprocessed_data = self .preprocessed_data,
                                                                        preprocessed_data_6kmph = self .preprocessed_data_6kmph)
